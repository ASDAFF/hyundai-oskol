...
//	        var mod_id = $($this).find('option').eq(0).val();
	...
	function initWidget() {
		...
		window.updateWidgetData = function (e) {
			...
			var g= document.querySelector('g'),
			    transformVar = getComputedStyle(g).getPropertyValue('transform');

			g.setAttribute('transform', transformVar);
		}
	...
	function initSlider() {
		...
		function sliderStart() {
			...
			$(classes.slider.next).unbind().bind('click', nextSlide);
			$(classes.slider.prev).unbind().bind('click', prevSlide);
...
//				getModificationList(dataId);
	...
	{
		$.post('/requestnew/getpcpmod', 'carid=' + car_id, function(res){
...
*/
	...
	{
		$.post('/requestnew/getpcpcompl', 'carid=' + car_id + '&modid=' + mod_id, function(res){
...
$(function(){
			...
			this.$container.find('[data-form-id="start-credit-form"]')
				.validit({
					onSuccessSubmit: function(){
					    ...
					    var carid = getCarId();
					    var mod = '2.0(149) MPi - 6AT - 2WD';
					    var compl = 'Active';
					    var pay = parseInt($('.calc__info .startprice').html().replace(' ', '').replace(' ', '').replace('&nbsp;', '').replace('&nbsp;', ''));
						var term = 36;
	...
	$('#modal-legal .closeIt').click(function(){
		$('#modal-legal').fadeOut();
	});
