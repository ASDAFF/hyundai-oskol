...
<?						elseif(!empty($arField["AUTOCOMPLETE_VALUE"])):
							...
							<input type="text" size="40" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" <?=$readonly?>value="" placeholder="<?=$arField['NAME']?>" class="afbf_inputtext" />
<?						else:?>
							<input type="text" size="40" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" value="" placeholder="<?=$arField['NAME']?>" class="afbf_inputtext" onblur="if(this.value==''){this.value='<?=$arField["DEFAULT_VALUE"]?>'}" onclick="if(this.value=='<?=$arField["DEFAULT_VALUE"]?>'){this.value=''}" />
<?						endif;?>
...
<?		echo bitrix_sessid_post()?>
		<div class="afbf_submit_block">
			<input type="submit" class="fb_close afbf_btn" id="fb_close_<?=$ALX?>" name="SEND_FORM" value="Записаться на тест-драйв" />
		</div>
	</form>
</div>
